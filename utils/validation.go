...
// func dateComparison(fl validator.FieldLevel) bool {
// 	// Retrieve the field values
// 	fieldName := fl.Param()
// 	start := fl.Field().Interface()
// 	end := fl.Parent().FieldByName(fieldName).Interface()
// 	println(fl.Parent().Field(0).String())
// 	// Check if the values are pointers to time.Time and dereference them if needed
// 	if startPtr, ok := start.(*time.Time); ok {
// 		start = *startPtr
// 	}
// 	if endPtr, ok := end.(*time.Time); ok {
// 		end = *endPtr
// 	}

// 	// Perform the date comparison
// 	return start.(time.Time).Before(end.(time.Time))
// }
