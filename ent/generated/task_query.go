...
func (tq *TaskQuery) QueryAccount() *AccountQuery {
...
func (tq *TaskQuery) QueryIssues() *IssueQuery {
	query := (&IssueClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withIssues:    tq.withIssues.Clone(),
...
func (tq *TaskQuery) WithAccount(opts ...func(*AccountQuery)) *TaskQuery {
	query := (&AccountClient{config: tq.config}).Query()
...
func (tq *TaskQuery) WithIssues(opts ...func(*IssueQuery)) *TaskQuery {
	query := (&IssueClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withAccount; query != nil {
		if err := tq.loadAccount(ctx, query, nodes, nil,
			func(n *Task, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := tq.withIssues; query != nil {
		if err := tq.loadIssues(ctx, query, nodes, nil,
			func(n *Task, e *Issue) { n.Edges.Issues = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Task, init func(*Task), assign func(*Task, *Account)) error {
...
func (tq *TaskQuery) loadIssues(ctx context.Context, query *IssueQuery, nodes []*Task, init func(*Task), assign func(*Task, *Issue)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i].TaskIssueID == nil {
			continue
		}
		fk := *nodes[i].TaskIssueID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_issue_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withAccount != nil {
			_spec.Node.AddColumnOnce(task.FieldAccountID)
		}
		if tq.withUser != nil {
		...
		if tq.withIssues != nil {
			_spec.Node.AddColumnOnce(task.FieldTaskIssueID)
		}
	}
